...
func (vm *VM) Initialize(
	...
	if err != nil {
		log.Error("error initializing Medibase VM: %v", err)
		return err
	}
	log.Info("Initializing Medibase VM", "Version", version)
	...
	if !vm.DBInitialized() {
		...
		genesisBlock, err := vm.NewBlock(ids.Empty, 0, strconv.Itoa(0), genesisDataArr, time.Unix(0, 0))
		if err != nil {
...
func (vm *VM) BuildBlock() (snowman.Block, error) {
	...
	block, err := vm.NewBlock(vm.Preferred(), preferredHeight+1, value.PayID, value.CID, time.Now())
	if err != nil {
...
func (vm *VM) proposeBlock(payID string, cid [dataLen]byte) {
	data := BlkData{PayID: payID, CID: cid}
...
func (vm *VM) NewBlock(parentID ids.ID, height uint64, payID string, cid [dataLen]byte, timestamp time.Time) (*Block, error) {
